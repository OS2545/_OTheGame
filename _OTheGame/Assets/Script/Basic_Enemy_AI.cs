...
public class Basic_Enemy_AI : MonoBehaviour
{
    ...
    private void Update()
    {
        if(isPlaying){
            if(TryPlaying()){
                isPlaying=false; //finish playing
            }
        }
    }
    ...
    private bool TryPlaying(){
       ...
       AI_Margin margin = new AI_Margin(0,null);

        foreach(XO_Item xO_Item in AllXOList){
            if(ActiveXOList.Contains(xO_Item)) continue; //not calculate margin for taken spot
            ActiveXOList.Add(xO_Item); //mock up active XO tile
            margin=CalculateMargin(margin,xO_Item,ActiveXOList); //calculate that xo
            ActiveXOList.RemoveAt(ActiveXOList.Count-1); //remove mock up
        }
    ...
    private double Calculate_Extra_AIPlayerHit(int AiHit, int playerHit){
        double point =0;
        if(playerHit>0) point +=0.05; //Encourage to block player
        //if(AiHit>0) point+=1;
